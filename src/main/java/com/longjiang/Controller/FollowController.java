...
@Controller
public class FollowController implements LongJiangConstant{
    @Autowired
    ...
    @ResponseBody
    @LoginRequired
    public  String follow(int entityType,int entityId) throws Exception {
        User user = baseContext.getUser();
        followService.follow(user.getId(),entityType,entityId);
        //触发关注事件
        Event event = new Event().setTopic(TOPIC_FOLLOW).setUserId(baseContext.getUser().getId())
                .setEntityType(entityType).setEntityId(entityId).setEntityUserId(entityId);
        eventProducer.fireEvent(event);
        return LongJiangUtil.getJSONString(0,"已关注");
    }
    ...
    public String unfollow(int entityType,int entityId){
        ...
        return LongJiangUtil.getJSONString(0,"已取消关注");
    }
    ...
    private boolean hasFollowed(int userId){
        ...
        return followService.hasFollowed(baseContext.getUser().getId(),ENTITY_TYPE_USER,userId);
    }
}
